<?php
    ...
    $sql = "SELECT * FROM `survey_result` WHERE `Survey_ID` = '$Survey_ID' AND `Teacher` = '1' AND `Hide`='0'";
    $result = mysqli_query($con,$sql);
    ...
    $ResultName="teacher_survey_results_".$Survey_ID."_".date("Y_m_d").".xlsx";
    $writer->writeToFile("./xlsx/".$ResultName);
    $sql = "UPDATE `course_list` SET `teacher_survey_excel_name`='$ResultName' WHERE `Survey_ID` = '$Survey_ID'";
    mysqli_query($con,$sql);
    //echo "show_attendence_list.php";
    echo "<meta http-equiv=REFRESH CONTENT=0.01;url=show_teacher_survey_results.php>";
?>