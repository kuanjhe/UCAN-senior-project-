...
<?php
  //if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_SESSION['survey_student_id'])){
      if ($_SESSION['survey_student_id']!=''){
       // $_SESSION['survey_student_id']=$_POST['survey_student_id'];
        $sql = "SELECT * FROM `survey_result` WHERE `Course_ID`='{$Course_ID}' AND `StudentID`='{$_SESSION['survey_student_id']}' AND `Teacher` IS NULL";
        $result = mysqli_query($con,$sql);
...
<?php
  ...
  if(isset($_SESSION['o5H4KY3Uz2'])){
    ...
    $sql = "SELECT * FROM survey_item_name WHERE `Course_ID`='$Course_ID' AND `Survey_Teacher` IS NULL";
    $result = mysqli_query($con,$sql);
    for($i=1; $i<=min(mysqli_num_rows($result),$number_survey_items); $i++){
      $row = mysqli_fetch_assoc($result);
      $_SESSION['survey_item_no_id'][]=$row['ID'];
    }
    if((!isset($_SESSION['o5H4KY3Uz2789'])&&empty($_SESSION['survey_student_id']))||empty($_SESSION['survey_student_id'])){
...
<?php
    ...
    if ($number_survey_items<=count($_SESSION['survey_item_results'])) {
      ...
      $sql ="INSERT INTO `survey_result` (`ID`, `Course_ID`, `StudentID`, `Time`, `Hide`) VALUES (NULL, '$Course_ID', '{$_SESSION['survey_student_id']}','$record_ime', '0')";
...
?>
          <h2><?php if(isset($_SESSION['survey_student_id'])) echo "學生".$_SESSION['survey_student_id']?></h2><h2>第<?php echo ($survey_item_no+1)?>題</h2>
<?php   
  $sql = "SELECT * FROM survey_item_name WHERE `Course_ID`='$Course_ID' AND `ID`='{$_SESSION['survey_item_no_id'][$survey_item_no]}'";
  $result = mysqli_query($con,$sql);
  $row = mysqli_fetch_assoc($result);
?>   
...
?>
          ...
          <p><a href="reset_survey_result.php?Course_ID=<?php echo $Course_ID?>" class="btn btn-info">重新填寫</a></p>
<?php
    }
  }
