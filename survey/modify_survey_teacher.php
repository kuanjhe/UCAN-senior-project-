<?php
    ...
    if (!isset($_SESSION['Course_ID'])){
      ...
      $number_survey_teacher_items=$row['number_survey_teacher_items'];
      $sql = "SELECT `Survey_Teacher` FROM `course_list` WHERE `Course_ID`='{$Course_ID}'";
      $result = mysqli_query($con,$sql);
      $row = mysqli_fetch_assoc($result);
      if ($row['Survey_Teacher']==0){
      echo "<meta http-equiv=REFRESH CONTENT=0;url=../add_course_survey.php>";
      }
    }
?>
<!DOCTYPE html>
<html>
<title>修改<?php echo $site_name?>教師問卷</title>
    <meta charset="utf-8">
...
<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		if (isset($_POST['number_survey_teacher_items'])){
			$sql = "UPDATE `course_list` SET `number_survey_teacher_items`='{$_POST['number_survey_teacher_items']}' WHERE `Course_ID`='{$Course_ID}'";
		...
		if (isset($_POST['optradio'])){
        	...
        	for($j=0; $j<count($ID); $j++){
    	   		...
    	   		$sql = "UPDATE `survey_item_name` SET `Problem_CName`='$Problem_CName', `item_type`='$item_type',`number_items`='$number_items' WHERE `ID`='$ID[$j]' AND `Survey_Teacher`='1' ".$additional_string;
    	   		mysqli_query($con,$sql);
    	   		$sql2 = "SELECT * FROM `survey_item_list` WHERE `survey_item_id`='$ID[$j]' AND `Survey_Teacher`='1' ".$additional_string;
    	   		//echo $sql2;
            	...
            	if (mysqli_num_rows($result2)<$number_items){
                	for($i=1; $i<=$number_items-mysqli_num_rows($result2); $i++){
                    	$sql3 = "INSERT INTO `survey_item_list`(`ID`,`survey_item_id`, `Option_CName`,`Course_ID`,`Survey_Teacher`) VALUES (NULL,'$ID[$j]',NULL,'$Course_ID','1')";
		...
		if (isset($_POST['optradioB'])){
			...
			for($i=0; $i<count($ID); $i++){
        		$Option_CName = "Option_CName_".$ID[$i];
    	   		...
    	   		$sql = "UPDATE `survey_item_list` SET `Option_CName`='$Option_CName' WHERE `ID`='{$ID[$i]}' AND `Survey_Teacher`='1' AND `Course_ID` = '$Course_ID'";
    	   		mysqli_query($con,$sql);
    	   	}
		}

	}
	$sql = "SELECT `number_survey_teacher_items` FROM `course_list` WHERE `Course_ID`='{$Course_ID}'";
	$result = mysqli_query($con,$sql);
	$row = mysqli_fetch_assoc($result);
	$number_survey_teacher_items=$row['number_survey_teacher_items'];
?>
...
?>
	<div class="container">
		...
		<form action="" method="post">
            <div class="form-group">
                ...
                <select name="choose" id="choose" required>
                    <option value="1">學生問卷</option>
                    <option value="0">教師問卷</option>
                </select></p>
            </div>
        ...
        <?php
        		...
        		if($_SESSION['choose']==0){
        ?>	
      
		<form action="" method="post">
			<div class="form-group">
				<label for="number_survey_teacher_items">問卷題目總數:</label>
				<input type="text" class="form-control" name="number_survey_teacher_items" id="$number_survey_teacher_items" value="<?php echo $number_survey_teacher_items?>" required>
			</div>
		...
		<table class="table table-bordered table-hover table-condensed" style="background-color:lavender;">
			<tbody>
				<tr><th style="text-align: center" colspan=6>問卷清單(總數：<?php echo $number_survey_teacher_items?>)</th></tr>
				<tr><th style="text-align: center"></th><th style="text-align: center">問卷編號</th>
					<th style="text-align: center">問卷題目內容</th><th style="text-align: center">題目類型</th>
					<th style="text-align: center">選項總數</th>
				<th></th>
				</tr>
			</tbody>
<?php
	$sql = "SELECT * FROM survey_item_name WHERE `Survey_Teacher` ='1' ".$additional_string;
	$result = mysqli_query($con,$sql);
	for($i=1; $i<=min(mysqli_num_rows($result),$number_survey_teacher_items); $i++){
		$row = mysqli_fetch_assoc($result);
		echo "<tr>\n";
		...
		echo "<td style=\"text-align: center\"><input type=\"text\" class=\"form-control\" name=\"Problem_CName{$row['ID']}\" id=\"Problem_CName{$row['ID']}\" placeholder=\"請輸入問卷題目內容\" value=\"{$row['Problem_CName']}\"";
		echo " onclick=\"radio_checked('{$row['ID']}')\" onselect=\"radio_checked('{$row['ID']}')\" required>";
		echo "</td>\n";
		echo "<td style=\"text-align: center\" ><select class=\"form-control\" name=\"item_type{$row['ID']}\" id=\"item_type{$row['ID']}\" onclick=\"radio_checked('{$row['ID']}')\" onselect=\"radio_checked('{$row['ID']}')\" required>";
...
<?php
			$sql3 = "SELECT * FROM `survey_item_list` WHERE `survey_item_id` = '{$row['ID']}' AND `Course_ID` = '$Course_ID' AND `Survey_Teacher` ='1'";
...
<?php
	...
	echo "<meta http-equiv=REFRESH CONTENT=0;url=modify_survey.php>";
}
}
?>
    </div>
