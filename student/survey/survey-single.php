<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST'){
   if(isset($_POST['Course_CName'])){
    $_SESSION['Course_CName']=$_POST['Course_CName'];
   }
      $sql = "SELECT `Survey_ID`FROM `course_list` WHERE `Course_CName`='".$_SESSION['Course_CName']."'";
      $result = mysqli_query($con,$sql);
      $row = mysqli_fetch_row($result);
      $_SESSION['Course_ID']=$row[0];
      $Course_ID =$row[0];

      $sql = "SELECT `Course_CName`,`number_survey_items` FROM `course_list` WHERE `Survey_ID`='$Course_ID'";
      $result = mysqli_query($con,$sql);
      $row = mysqli_fetch_row($result);
      $site_name=$row[0];
...
<?php
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $sql = "SELECT * FROM `survey_result` WHERE `Survey_ID`='{$Course_ID}' AND `UserID`='{$_SESSION['Username']}'";
        $result = mysqli_query($con,$sql);
    ...
    if (isset($_POST['comment'])){
      ...
      $_SESSION['survey_item_results'][]="無";
    }
    if (isset($_POST['fill'])){
      if ($_POST['fill']!='')
...
<?php
  ...
  if ($_SESSION['login_RNuikEEFpDrrjTQ999']=="PIYW7iyWXS6POakVFnzPik80xJquSWHaePSqnYdh999"){
      ...
      echo "<a href=../check_courses.php>確認課程</a>\r";
      echo "<a href=../choose_courses.php>填寫問卷</a>\r";
      echo "<a href=../create_radar.php>查詢各課程評量分數</a>\r";
...
<?php
  if (!isset($_SESSION['survey_item_results'])){
    ...
    $sql = "SELECT * FROM survey_item_name";
    $result = mysqli_query($con,$sql);
...
<?php
    if ($number_survey_items<=$_SESSION['quction']) {
      ...
      $sql ="INSERT INTO `survey_result` (`ID`, `Survey_ID`, `UserID`, `Time`, `Hide`) VALUES (NULL, '$Course_ID', '{$_SESSION['Username']}','$record_ime', '0')";
      mysqli_query($con,$sql);

      $sql = "SELECT `Course_ID` FROM `course_list` WHERE `Survey_ID`='$Course_ID'";
      $result = mysqli_query($con,$sql);
      $row = mysqli_fetch_row($result);

      $sql = "UPDATE `student_list` SET Survey= 1 WHERE Course_ID = '{$row[0]}' AND StudentID = '{$_SESSION['Username']}'";
      mysqli_query($con,$sql);
      $sql = "UPDATE `survey_result` SET `item_".sprintf('%02d',1)."`='{$_SESSION['survey_item_results'][0]}'";
      for ($i=2; $i < $number_survey_items; $i++) {
        $sql =$sql.", `item_".sprintf('%02d',$i)."`='{$_SESSION['survey_item_results'][$i]}'";
      }
      $sql = $sql ." WHERE `Survey_ID`='$Course_ID' AND `UserID`='{$_SESSION['Username']}'";
      mysqli_query($con,$sql);
  ...
  $sql = "SELECT * FROM survey_item_name WHERE `ID`='{$_SESSION['Usernamer'][$_SESSION['quction']]}'";
  $result = mysqli_query($con,$sql);
  $row = mysqli_fetch_assoc($result);
  ...
  if($item_type==3){
        ...
        echo "<textarea class=\"form-control form-control-lg\" rows=\"5\" name=\"comment\" id=\"comment\">請在此輸入建議</textarea>";
        echo "</div>";
  } 
  if ($item_type==4){
  ...
  if ($item_type==6){
    ...
    for ($i=0; $i < $number_items; $i++) {
      ...
      echo "<input type=\"text\" class=\"form-control form-control-lg\" name=\"additional6{$j}\" id=\"additional6{$j}\" value=\"無\" style=\"visibility:hidden\">\n";
    }
  }
echo" </p>
