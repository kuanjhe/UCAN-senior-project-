<?php
    ...
    if (!isset($_SESSION['Course_ID'])){
      ...
      $sql = "SELECT `Survey` FROM `course_list` WHERE `Course_ID`='{$Course_ID}'";
      $result = mysqli_query($con,$sql);
      $row = mysqli_fetch_row($result);
      if ($row[0]==0){
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		if (isset($_POST['optradio'])){
        	...
        	for($j=0; $j<count($ID); $j++){
    	   		$Course_CName = "Course_CName".$ID[$j];
    	   		...
    	   		$sql = "UPDATE `survey_item_name` SET `Course_CName`='$Course_CName', `item_type`='$item_type',`number_items`='$number_items' WHERE `ID`='$ID[$j]' ".$additional_string;
    	   		mysqli_query($con,$sql);
    	   		$sql2 = "SELECT * FROM `survey_item_list` WHERE `survey_item_id`='$ID[$j]'".$additional_string;
    	   		//echo $sql2;
            	...
            	if (mysqli_num_rows($result2)<$number_items){
                	for($i=1; $i<=$number_items-mysqli_num_rows($result2); $i++){
                    	$sql3 = "INSERT INTO `survey_item_list`(`ID`,`survey_item_id`, `Option_CName`,`Course_ID`) VALUES (NULL,'$ID[$j]',NULL,'$Course_ID')";
		...
		if (isset($_POST['optradioB'])){
    	   ...
    	   //$Numbers = $_POST[$Numbers];
    	   		$sql = "UPDATE `survey_item_list` SET `Option_CName`='$Option_CName' WHERE `ID`='{$ID[$i]}' ".$additional_string;
    	   		mysqli_query($con,$sql);
    	   	}
		}
	}
	...
	<div class="container">
		...
		<form action="" method="post">
			<div class="form-group">
				<label for="number_survey_items">問卷題目總數:</label>
...
<?php
	...
	for($i=1; $i<=min(mysqli_num_rows($result),$number_survey_items); $i++){
		...
		echo "<td style=\"text-align: center\"><input type=\"text\" class=\"form-control\" name=\"Course_CName{$row['ID']}\" id=\"Course_CName{$row['ID']}\" value=\"{$row['Course_CName']}\"";
		echo " onclick=\"radio_checked('{$row['ID']}')\" onselect=\"radio_checked('{$row['ID']}')\" required>";
		echo "</td>\n";
		echo "<td style=\"text-align: center\" ><select class=\"form-control\" name=\"item_type{$row['ID']}\" id=\"item_type{$row['ID']}\"";
		$item_type_array=array('選擇題','是非題','申論題','選擇題','多選題','選擇+其他');

		for($j=0; $j<count($item_type_array); $j++){
			echo "<option value=\"".$j."\"";
		...
		/*for($j=0;$j<count($item_type_array);$j++){
			echo "<option value=\"".$j."\">{$item_type_array[$j]}</option>\n";
		}*/
		echo " onclick=\"radio_checked('{$row['ID']}')\" onselect=\"radio_checked('{$row['ID']}')\" required>";
		echo "</select>";
		echo "</td>\n";
...
<?php
	$sql = "SELECT * FROM `survey_item_name` WHERE 1".$additional_string;
	$result = mysqli_query($con,$sql);
	for($i=1; $i<=min(mysqli_num_rows($result),$number_survey_items); $i++){
		$row = mysqli_fetch_assoc($result);
		if (($row['item_type']=='1') or ($row['item_type']=='4') or ($row['item_type']=='5') or ($row['item_type']=='6')){
?>
		<form id="myForm" action="" method="post">
		<table class="table table-bordered table-hover table-condensed" style="background-color:lavender;">
			<tbody>
				<tr><th></th><th style="text-align: center" colspan=4>第<?php echo $i?>題：<?php echo $row['Course_CName']?></th></tr>
				<tr><th></th><th style="text-align: center">選項序號</th><th style="text-align: center" >選項內容</th><th></th>
				</tr>
			</tbody>
<?php
			$sql3 = "SELECT * FROM survey_item_list WHERE survey_item_id = {$row['ID']}".$additional_string;
			...
			echo " onclick=\"radioB_checked('{$row3['ID']}')\" onselect=\"radioB_checked('{$row3['ID']}')\" required>";
			echo "</td>\n";
			//echo "<td style=\"text-align: center\" ><input type=\"text\" class=\"form-control\" name=\"item_type{$row['ID']}\" id=\"item_type{$row['ID']}\" value=\"{$row['item_type']}\"";
			//echo " onclick=\"radio_checked('{$row['ID']}')\" onselect=\"radio_checked('{$row['ID']}')\" required>";
			//echo "</td>\n";
