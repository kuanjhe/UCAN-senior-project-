var RadarChart = {
  draw: function(id, d, options){
    ...
    axis.append("text").attr("class", "legend")
        ...
        .attr("y", function(d, i){return 0.91*getVerticalPosition(i, cfg.h / 2, cfg.factorLegend)+30;});
    ...
    d.forEach(function(y, x){
      ...
      g.selectAll(".nodes")
        .data(y, function(j, i){
          dataValues.push([
            getHorizontalPosition(i, cfg.w/2, (parseFloat(Math.max(j.value, 0))/j.maxvalue)*cfg.factor),
            getVerticalPosition(i, cfg.h/2, (parseFloat(Math.max(j.value, 0))/j.maxvalue)*cfg.factor)
          ]);
        });
    ...
    d.forEach(function(y, x){
      g.selectAll(".nodes")
        ...
        .attr("cx", function(j, i){
          ...
          return getHorizontalPosition(i, cfg.w/2, (Math.max(j.value, 0)/j.maxvalue)*cfg.factor);
        })
        .attr("cy", function(j, i){
          return getVerticalPosition(i, cfg.h/2, (Math.max(j.value, 0)/j.maxvalue)*cfg.factor);
        })
